...
SKRL_VERSION = "1.4.3"
if version.parse(skrl.__version__) < version.parse(SKRL_VERSION):                                                   # type: ignore[arg-type]
    skrl.logger.error(                                                                                              # type: ignore[arg-type]
        f"Unsupported skrl version: {skrl.__version__}. "                                                           # type: ignore[arg-type]
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    if args_cli.ml_framework.startswith("jax"):
        skrl.config.jax.backend = "jax" if args_cli.ml_framework == "jax" else "numpy"                              # type: ignore[arg-type]
    ...
    env = SkrlVecEnvWrapper(env, ml_framework=args_cli.ml_framework)  # same as: `wrap_env(env, wrapper="auto")`    # type: ignore[arg-type]

    # configure and instantiate the skrl runner
    runner = Runner(env, agent_cfg)                                                                                 # type: ignore[arg-type]
