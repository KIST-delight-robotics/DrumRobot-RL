...
- **양팔 조합**: 양손에 각각 목표 드럼이 할당되는 38가지 조합 중 episode마다 하나가 선택됨 (물리적으로 동시 타격 가능한 조합)
...
| 지표 | 왼손 | 오른손 | 의미 |
|---|---|---|---|
| 타겟까지 xy 거리 | 0.049 m | 0.040 m | 평면 정렬 정밀도 |
| 타겟까지 z 오차 | 0.045 m | 0.043 m | 높이 정렬 정밀도 |
| 타격 준비 상태 진입 비율 | 13.5% | 27.2% | 내려치는 사이클에 들어간 시간 비율 |
| 첫 타격 성공률 (`first_hit`) | ≈ 0 | ≈ 0 | 인정된 타격 발생 빈도 |

여기에 더해 보상 항으로 본 동작의 비대칭:

- **하향 속도 보상**: 0.164
- **상향 속도 보상**: 0.045

이 숫자들을 같이 읽으면 정책이 학습한 동작이 분명하게 드러난다.

**도달은 됐다.** 양손이 각자 할당된 드럼 위에 4~5cm 이내로 정밀하게 위치한다. 8개 드럼 × 38개 양팔 조합을 one-hot으로만 구분하는 관측 설계가 실제로 작동한다는 뜻이다.

**그러나 타격은 안 됐다.** `first_hit`이 사실상 0이라는 건, 700K step 동안 판정 기준을 통과한 타격이 거의 한 번도 발생하지 않았다는 의미다. 타격 준비 상태에 들어가는 시간(왼손 13%, 오른손 27%)이 0이 아니라는 점은, 정책이 내려치는 시도 자체는 한다는 뜻이지만, 거기서 끝난다.

**어디서 끊겼는지도 보인다.** 하향 속도 보상(0.164)이 상향 속도 보상(0.045)의 약 3.6배다. 내려가는 동작은 만들지만, 튕겨 올라오는 동작을 거의 만들지 못한다. 타격 판정이 "내려갔다가 다시 올라온다"는 2단계 시퀀스를 요구하므로, 상승 단계가 빠진 상태에서는 타격이 인정되지 않는다.

영상으로 보면 양손이 드럼 표면 위에 정밀하게 도달해 그대로 머문다. 학습된 정책은 **"드럼 위에 수렴"**이지 **"드럼을 친다"**가 아니다.
