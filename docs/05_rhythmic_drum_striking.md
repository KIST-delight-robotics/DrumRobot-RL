...
100K(MIDI 사전학습 직후 fine-tune 시작) vs 1M(랜덤 RDS fine-tune 종료) 비교:
...
| 잘못 침 비율 | 23.1% | 44.7% |
| 놓침 비율 | 46.5% | 37.4% |
| Strike phase 보상 | 0.165 | 0.359 |
| 진행 보상 (×100) | 0.143 | 0.347 |

**탐색은 풀렸다.** Strike phase 보상과 진행 보상이 학습 동안 2배 이상으로 활성화됐다. 04번에서 진행 보상이 0.001로 죽어 있던 것과 비교하면, phase 보상 + 잘못 침 페널티 약화의 조합이 dense 학습 신호를 살리는 데 성공했다.

**그러나 정확도가 함께 떨어졌다.** 잘못 침이 23%에서 45%로 거의 두 배가 됐다. 정책이 "치자"의 모드로 옮겨갔지만, 시도의 절반이 엉뚱한 드럼이다. 100K 시점이 1M보다 성공률이 더 높은 역전 현상이 나타난다.

**드럼별 편차는 좁혀졌다.** 04번의 5배 편차(스네어 근처 vs 크래시)가 완화되어, 좌 크래시도 35% 수준으로 학습 분포에 들어왔다. 다만 플로어가 6%로 떨어진 점은 양손 분담이 정책 안에 명시되지 않아 잘못된 손이 가는 양상이 남아 있음을 시사한다.
...
성공률이 17%에 머문 시점에서 **1M step을 더 돌렸을 때 가파른 개선은 예상하기 어려운 상태**였다.
