...
9차원 연속 행동 `[-1, 1]`, `action_scale = 0.03` 적용 → 각 관절에 작은 위치 증분 명령. 안정성 우선으로 한 step당 변화량을 작게 잡음.
...
- 종료 페널티: 관절이 실제 하드 리미트를 넘어선 순간 큰 음의 보상 + episode 종료. early termination 신호
...
```
reward=1.196 | L_dist=0.090 | R_dist=0.157 | dist_sum=0.248
action_l2=5.834 | limit_pen=0.001 | died=0.001
success_rate=0.031
```
...
- **시작하자마자 관절 범위를 넘는 방향으로 학습됨**. 보상이 음수이고 성공을 경험하지 못하면 에피소드를 빠르게 끝내는 것이 유리해진다. 일반적인 상황에서 보상이 양수가 되도록 가중치를 조정.
...
- **soft barrier + early termination의 단계화**. 관절 제한을 부드럽게 학습시키려면 페널티 영역을 미리 두고, 그것을 넘었을 때 episode 종료로 강하게 잡는 두 단계 구조가 유효.